...
execute as @e[type= minecraft:ghast,        tag=ER.animal.alpha, tag=ER.animal.hitbox] if score @s df_id = #this.ID ER.sys run tag @s add this
execute as @e[type= minecraft:item_display, tag=ER.animal.alpha, tag=ER.animal.model ] if score @s df_id = #this.ID ER.sys run tag @s add this
execute as @e[type= minecraft:text_display, tag=ER.animal.alpha, tag=ER.animal.HPbar ] if score @s df_id = #this.ID ER.sys run tag @s add this
#> 변수 얻어오기
...
data modify storage minecraft:temp temp set value {animal : "alpha", OPTdistance : 30, damage : 5, attackTick : 12, attackDistance : 3}
