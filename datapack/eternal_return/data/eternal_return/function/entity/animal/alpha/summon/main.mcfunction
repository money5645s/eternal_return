...
scoreboard players operation #health ER.sys = #ER.animal.alpha.health ER.sys
scoreboard players operation #cooltime ER.sys = #ER.animal.alpha.cooltime ER.sys
execute as @e[type=minecraft:husk, tag= this, tag= ER.animal.root] run function eternal_return:entity/animal/alpha/summon/b1/code
...
execute on vehicle run function eternal_return:entity/animal/alpha/summon/b3/code
