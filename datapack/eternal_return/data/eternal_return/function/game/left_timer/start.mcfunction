...
execute if score left.sec time matches 1 if score left.tick time matches 1 run title @a[tag=player] times 5 15 5
...
execute if score left.sec time matches 1 if score left.tick time matches 15 run gamemode spectator @a
execute if score left.sec time matches 1 if score left.tick time matches 15 run tag @a[tag=player] add freeze
...
## 시작 5초 배너
execute if score start.tick time matches 119 run playsound minecraft:er.game.start master @a ~ ~ ~ 99 1
