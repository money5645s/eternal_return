...
execute if data storage tmp is[{type:"scheduled_restricted_zone"}] if entity @e[tag=scheduled_restricted_zone] if score #timer.day NUM matches ..4 run function eternal_return:send_msg {target:"@a",text:'[{"text":"1분 후에 [ "},{"selector":"@e[tag=scheduled_restricted_zone]","color":"white"},{"text":" ]이(가) 금지구역으로 지정됩니다."}]',color:"gold",timer:1}
execute if data storage tmp is[{type:"scheduled_restricted_zone"}] if entity @e[tag=scheduled_restricted_zone] if score #timer.day NUM matches 5.. run function eternal_return:send_msg {target:"@a",text:'[{"text":"30초 후에 [ "},{"selector":"@e[tag=scheduled_restricted_zone]","color":"white"},{"text":" ]이(가) 금지구역으로 지정됩니다."}]',color:"gold",timer:1}
...
# 시간별 메시지 보내기
execute if data storage tmp is[{type:"restricted_zone"}] if entity @e[tag=restricted_zone,tag=!fix_area_type] if score #timer.halfday NUM matches 1 run function eternal_return:send_msg {target:"@a",text:'[{"text":"[ "},{"selector":"@e[tag=restricted_zone,tag=!fix_area_type]","color":"#9E0010"},{"text":" ]이(가) 금지구역으로 지정되었습니다. 5초간 위성 레이더가 작동됩니다."}]',color:"gold",timer:1}
execute if data storage tmp is[{type:"restricted_zone"}] if entity @e[tag=restricted_zone,tag=!fix_area_type] if score #timer.halfday NUM matches 2 run function eternal_return:send_msg {target:"@a",text:'[{"text":"[ "},{"selector":"@e[tag=restricted_zone,tag=!fix_area_type]","color":"#9E0010"},{"text":" ]이(가) 금지구역으로 지정되었습니다. 항공보급이 도착합니다."}]',color:"gold",timer:1}
# 오류 메시지
