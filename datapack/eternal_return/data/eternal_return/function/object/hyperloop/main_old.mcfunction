...
# 화면 선택 시 좌클릭
execute as @e[tag=hyperloop,tag=interaction] at @s on attacker if entity @s[tag=hyper_loop,distance=..2,tag=!loop] run function eternal_return:sys/hyperloop/display
# 클릭 NBT 태그 지우기
...
## 하이퍼루프에서 멀어질 때
execute if entity @e[tag=hyperloop,tag=interaction,distance=1..10] run function eternal_return:sys/hyperloop/remove/tag