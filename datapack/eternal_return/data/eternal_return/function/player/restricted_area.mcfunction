...
execute if score @s[tag=in_restricted_area] bantime matches 1.. if score @s bantime.tick matches 20 run title @s title {"text":"r","font":"screen_effect","shadow_color":0}
execute if score @s[tag=in_restricted_area] bantime matches 2.. if score @s bantime.tick matches 20 run playsound minecraft:er.restricted_area.alarm_tick master @s ~ ~ ~
