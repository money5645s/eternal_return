...
execute as @e[\
    tag= aj.animal_bear.animation.ready,\
    tag= ER.animal.model,\
    tag= this] run function animated_java:animal_bear/animations/ready/stop
...
execute at @e[tag= this,tag= ER.animal.attackDetection] \
    ...
    tag=!aj.animal_bear.animation.skill,\
    tag= ER.animal.model,\
    tag= this,\
    ] if entity @p[distance=0..1] run function eternal_return:entity/animal/bear/script/attack
...
#>    tag= ER.animal.model,\
#>    tag= this,\
#>    ] if entity @p[distance=0..1] run function eternal_return:entity/animal/bear/script/skill
...
execute if score #MotionExist ER.sys matches 1 run execute as @e[\
    ...
    tag=!aj.animal_bear.animation.skill,\
    tag= ER.animal.model, \
    tag= this] run function animated_java:animal_bear/animations/move/play
