...
execute at @s if entity @s[tag= !choosing_hyper_loop] run function eternal_return:maps/hyper_loop/show

#> 플레이어 커서 위치에 따른 버튼 감지 함수
function eternal_return:maps/hyper_loop/get_selected_place

#> 플레이어의 스코어가 위치를 지시하는 스코어와 같은 경우 :
execute if score @s ER.sys = #ER.place.factory ER.sys run say hi