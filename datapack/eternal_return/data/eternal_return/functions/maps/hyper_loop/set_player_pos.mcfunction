...
$execute if score $(scoreboardX) ER.sys matches 100..255 run \
    data modify storage minecraft:temp temp merge value {$(posX) : "$(x)"}
$execute if score $(scoreboardX) ER.sys matches 256.. run \
    data modify storage minecraft:temp temp merge value {$(posX) : "256"}
...
$execute if score $(scoreboardY) ER.sys matches 100..256 run \
    data modify storage minecraft:temp temp merge value {$(posY) : "$(y)"}
$execute if score $(scoreboardY) ER.sys matches 256.. run \
    data modify storage minecraft:temp temp merge value {$(posY) : "256"}