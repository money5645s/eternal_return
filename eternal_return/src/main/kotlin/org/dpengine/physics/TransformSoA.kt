...
class TransformSoA(size : Int) : SoAModule(size){
    ...
    internal val position = Vec3SoA(size); //위치
    internal val rotation = Vec3SoA(size); //몸 자체의 회전
    internal val direction = Vec3SoA(size); //바라보는 방향
    internal val velocity = Vec3SoA(size);
    internal val isModifiedVelocity = BooleanArray(size){false};
    internal val isModifiedPosition = BooleanArray(size){false};
    internal val actors = Array<MonobehaviourActor?>(size){null};
    /**
     * 위치벡터 [x, y, z], 각도(호도법 : -180 ~ 180) [rotX, rotY, rotZ]를 전달받아 Handle 생성 & 반환
     * */
    fun create(actor : MonobehaviourActor,x : Double, y : Double, z : Double, rotX : Double, rotY : Double, rotZ : Double) : Long{
