...
abstract class DPEntity(
    ...
    override fun lateinit(){
        transformHandle = erEngine.transformSoA.create(this,
    ...
    fun getDirection(): Vector3 {
        ...
        val xz = cos(radY)
        return this.matVecCalculator.vec3(-xz * sin(radX), -sin(radY), xz * cos(radX));
    }
    ...
    open fun getPosition(): Vector3 {
        val location = entity.location
        return this.matVecCalculator.vec3(location.x, location.y, location.z)
    }

    open fun setPosition(pos : Vector3) {
        val x = matVecCalculator.x(pos);
        val y = matVecCalculator.y(pos);
        val z = matVecCalculator.z(pos);
        setPosition(x, y, z);
    }
    ...
    open fun getVelocity() : Vector3{
        ...
        return this.matVecCalculator.vec3(v.x, v.y, v.z);
    }

    open fun setVelocity(vec : Vector3){
        val x = matVecCalculator.x(vec);
        val y = matVecCalculator.y(vec);
        val z = matVecCalculator.z(vec);
        setVelocity(x, y, z);
    }
     ...
     * */
    open fun damage(amount : Double, attacker : DPEntity, damageType : DamageType){
     ...
     * */
    open fun damageForce(amount : Double, attacker : DPEntity, damageType : DamageType){
        if(entity is LivingEntity){
     ...
     * */
    open fun damageForcePierce(amount : Double, attacker : DPEntity){
        if(entity is LivingEntity){
            entity.noDamageTicks = 0;
            this.submitEvent(EREntityDamagedEvent(attacker))
            attacker.submitEvent(EREntityAttackEvent(attacker, this))
     ...
     * */
    open fun damageNotSendEvent(amount : Double, attacker : DPEntity){
        if(entity is LivingEntity){
            entity.damage(amount, attacker.entity); //이것도 특수한 SoA 함수로 뺄 것
        }
    }
     ...
     * 피해를 주는 메소드, 무적시간에 영향 받음, MonobehaviourEvent를 전달하지 않음.
     * */
    open fun damageNotSendEventPierce(amount : Double, attacker : DPEntity){
        if(entity is LivingEntity){
            entity.noDamageTicks = 0;
