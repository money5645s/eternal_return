...
$execute at @e[tag= this,tag= ER.animal.attackDetection] \
    if entity @e[tag=   aj.animal_$(animal).animation.attack,\
                 tag=  !aj.animal_$(animal).animation.skill,\
                 tag=   ER.animal.model,tag= this,\
                 scores= {aj.anim_time = 12}] run \
    function eternal_return:entity/animal/class/attack {animal : "$(animal)", damage : $(damage)}
...
#>공격 중엔 그대로 멈춰라
$execute if entity @e[tag= aj.animal_$(animal).animation.attack, tag= ER.animal.model,tag= this] run data modify entity @s Motion set value [0.0d,0.0d,0.0d]


#>스킬 중엔 그대로 멈춰라
$execute if entity @e[tag= aj.animal_$(animal).animation.skill,  tag= ER.animal.model,tag= this] run data modify entity @s Motion set value [0.0d,0.0d,0.0d]


#>MOVE 애니메이션 종료
$execute if score #MotionExist ER.sys matches 0 as @e[tag= aj.animal_$(animal).animation.move,tag= ER.animal.model,tag= this] run function animated_java:animal_$(animal)/animations/move/stop
...
#>MOVE 애니메이션 실행
$execute if score #MotionExist ER.sys matches 1 as @e[tag=!aj.animal_$(animal).animation.move,tag=!aj.animal_$(animal).animation.attack,tag=!aj.animal_$(animal).animation.skill,tag= ER.animal.model, tag= this] run function animated_java:animal_$(animal)/animations/move/play
