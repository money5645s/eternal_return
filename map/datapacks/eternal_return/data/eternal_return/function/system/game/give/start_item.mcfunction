...
execute as @a[tag=player] run item replace entity @s container.2 with bread[food={nutrition:1,saturation:1,can_always_eat:true,eat_seconds:1,effects:[{effect:{id:"minecraft:regeneration",amplifier:0,duration:200,show_particles:0b,show_icon:1b},probability:1}]}] 3
execute as @a[tag=player] run item replace entity @s container.7 with arrow 5
